...
if(isset($_POST['cancel'])){
   ...
   if($verify_booking->rowCount() > 0){
      ...
      $success_msg[] = 'Pesan Kamar berhasil dibatalkan!';
   }else{
      $warning_msg[] = 'Pesan kamar telah dibatalkan!';
   }
...
<section class="bookings">
   ...
   <h1 class="heading">Tiket Pesanan Saya</h1>
   ...
   ?>
   <div class="box">
      <p>nama : <span><?= $fetch_booking['name']; ?></span></p>
      <p>email : <span><?= $fetch_booking['email']; ?></span></p>
      <p>nomor : <span><?= $fetch_booking['number']; ?></span></p>
      <p>check in : <span><?= $fetch_booking['check_in']; ?></span></p>
      <p>check out : <span><?= $fetch_booking['check_out']; ?></span></p>
      <p>kamar : <span><?= $fetch_booking['rooms']; ?></span></p>
      <p>orang dewasa : <span><?= $fetch_booking['adults']; ?></span></p>
      <p>anak-anak : <span><?= $fetch_booking['childs']; ?></span></p>
      <p>id pesanan : <span><?= $fetch_booking['booking_id']; ?></span></p>
      <form action="" method="POST">
         <input type="hidden" name="booking_id" value="<?= $fetch_booking['booking_id']; ?>">
         <input type="submit" value="Batalkan" name="cancel" class="btn" onclick="return confirm('batalkan pesanan tiket?');">
      </form>
   </div>
   <?php
    }
   ...
   <div class="box" style="text-align: center;">
      <p style="padding-bottom: .5rem; text-transform:capitalize;">Pesanan Tiket Tidak ditemukan!</p>
      <a href="halaman.php" class="btn">Tiket baru</a>
   </div>
   <?php
   }
   ?>
